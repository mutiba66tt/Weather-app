...
const Signup = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err?.code?.replace('auth/', '').replace(/-/g, ' ') || 'Signup failed');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen sky-gradient flex items-center justify-center p-4">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          <Cloud className="w-12 h-12 text-primary mb-3" />
          <h1 className="text-3xl font-bold text-gradient">Create account</h1>
          <p className="text-sm text-muted-foreground mt-1">Join SkyWeather today</p>
        </div>
